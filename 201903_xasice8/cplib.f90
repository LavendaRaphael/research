...
!
      ...
      USE control_flags,      ONLY: iprint, iprsta, thdyn, tpre, tfor, &
                                    tprnfor, iesr, textfor, lwfcohsex, lwfHF  ! Lingzhu Kong
      USE io_global,          ONLY: stdout
...
!
!     ===================================================================
!                  Lingzhu Kong, COHSEX calculation
!     ===================================================================

      IF (lwfcohsex) THEN
         CALL vcoh( nspin, rhor )
      ELSEIF(lwfHF)THEN
         print *, 'HF, no vcoh'
         rhor = 0.d0
      ELSE
...
!$omp parallel do
            ...
            IF(lwfcohsex .or. lwfHF)THEN
              print *, 'COHSEX not compatible with abivol/abisur,stop'
              return
            ENDIF
         else
!$omp parallel do
...
!=============================================================================
! COHSEX calculation, modified from Wei Chen and Xifan's code by Lingzhu Kong

!  compute V_COH(r) in Bechstedt et al. 1992. (Sol.Stat.Comm. 84. 765)
!  input rho(r), for uniform e-gas, rho(r) = N_e / V
!  parameters for model dielctric function: alpha, epsilon0
!  everything is in a.u.
!=============================================================================

      SUBROUTINE vcoh(nspin, rhor)
      ...
      IMPLICIT none
      !rhor contains charge density on real space grid on input, vcoh on output
      INTEGER  :: nspin
      REAL(DP) :: rhor( nnr, nspin )

      INTEGER  i, iss
      REAL(DP) alpha, epsilon0,fac0, fac1, fac2,three, onesixth, rhotmp
      parameter ( three=3.D0, onesixth = 1.D0/6.D0 )

      ! Workspace for Simpson integration, added by Jianping Wang
      INTEGER  j, k, w_size
      REAL(DP) onethird, step, x, xstart, q, kf, epsilon_q
      REAL(DP) q_kf(6)  ! (q/kf)^n
      REAL(DP) ratio, coeff_a(6), coeff_b(6) ! energy-dependent correction factor

      PARAMETER ( w_size = 201, onethird = 1.D0/3.D0 )

      REAL(DP) w(w_size)

      DATA coeff_a /1.9085,  -0.542572, -2.45811, 3.08067,  -1.806,   0.410031/
      DATA coeff_b /2.01317, -1.55088,   1.58466, 0.368325, -1.68927, 0.599225/

      ! Workspace for Simpson integration, added by Jianping Wang

      IF (nspin > 1)THEN
         print *, 'spin polarization not implemented in COHSEX, STOP!'
         return
      ENDIF

!      alpha    = 2.3D0           ! 2.14 for water, 2.30 for ice !tianff
    alpha    = 1.242D0 !tianff
!      epsilon0 = 1.7D0           ! 1.8  for water, 1.7  for ice !tianff
    epsilon0 = 3.0D0 !tianff

!     fac0  = (three * pi * pi ) ** onesixth
!     fac1  = DSQRT( (1.D0 - 1.D0/epsilon0 ) /(pi*alpha) )
!     fac1  =-fac1 * fac0
!     fac2  = DSQRT( three * epsilon0 /(epsilon0 - 1.d0) )
!     fac2  = fac2 * 2.D0 /(alpha *sqrt(pi) * fac0 )

!     DO i = 1, nnr
!        rhotmp    = rhor(i,1) ** onesixth
!        rhor(i,1) = rhotmp *fac1 /DSQRT( 1.D0 + fac2 /rhotmp)
!     ENDDO

!     Added by Jianping Wang, COHSEX with frequency-dependence

!     (1) Analytical
!     fac0 = (three * pi * pi) ** onesixth
!     fac1 = DSQRT(epsilon0/(epsilon0-1.D0) * pi * 0.25) / fac0
!     fac2 = 4.D0 / pi * DSQRT(three)
!     DO i = 1, nnr
!        rhotmp = fac1 / rhor(i,1) ** onesixth
!        rho(i,1) = -0.5/(DSQRT(alpha + fac2*rhotmp) *rhotmp)
!     ENDDO

!     (2) Numerical
      WRITE(stdout,*) 'entering vcohsex'
      xstart = 1.0e-16
      step = (1.D0-xstart)/(w_size-1)
      fac0 = (three * pi * pi) ** onethird
      DO i = 1, nnr
         kf = fac0 * rhor(i,1) ** onethird

!         alpha = 2.0d0 * DSQRT(3.0d0) / DSQRT( pi * kf * (epsilon0-1.0d0)) ! tianff
         WRITE(stdout,*) i,'#tianff cplib.f90: alpha =', alpha ! tianff
         ...
         DO j = 1, w_size
            ...
            q_kf(1) = q/kf
            Do k = 2, 6
               q_kf(k) = q_kf(k-1)*q_kf(1)
            ENDDO
            ratio = ( 1.D0 + sum(q_kf*coeff_a) ) / ( 1.D0 + sum(q_kf*coeff_b) )
            epsilon_q = epsilon0/(epsilon0-1.D0) + alpha*0.25*pi*q**2/kf + 0.1875*pi*q**4/kf**3
            w(j) = w(j) * ratio / ( -epsilon_q * x**2 )
            x = x + step
         ENDDO

         rhor(i,1) = step/three/pi * sum(w)
      ENDDO
      WRITE(stdout,*) 'leaving vcohsex'
!     Added by Jianping Wang, COHSEX with frequency-dependence
