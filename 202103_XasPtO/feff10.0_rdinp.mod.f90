...
4245 continue  !Come back here to try again if the first list was too long
    ...
    end if  !If P or H, no need to make extra atoms
...
! ratx is now an array in Carthesian coordinates and in Angstrom units.
    ...
    tff_mindist = 0.d0
    do i = 1, nats
        tff_real_tmp = dsqrt((ppos(1, i) - ppos(1, absorber))**2 + (ppos(2, i) - ppos(2, absorber))**2&
                &       + (ppos(3, i) - ppos(3, absorber))**2)*alatt
        if (tff_mindist < tff_real_tmp) then
            tff_mindist = tff_real_tmp
        endif
    end do
